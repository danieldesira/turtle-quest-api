...
import { createProfilePicUrlMapFromHighScores, getHighScores, getPersonalBest, saveScore, } from "./services/scoreService.js";
import { deleteLastGame, getLastGame, getProfilePicKey, updateLastGame, updatePlayer, updatePlayerProfilePic, } from "./services/playerService.js";
import { checkAndRegisterPlayerGoogle, fetchGoogleUser, getJWTExpectedExpiry, } from "./services/authService.js";
import { sign } from "hono/jwt";
import { authMiddleware } from "./middleware.js";
import { Hono } from "hono";
import { deleteCookie, setCookie } from "hono/cookie";
import { zValidator } from "@hono/zod-validator";
import { gameUpdateSchema, loginSchema, playerUpdateSchema, pointInsertSchema, } from "./validation.js";
...
app.post("login", zValidator("json", loginSchema), async (c) => {
    ...
    const jwtToken = await sign({ id: player.id, email: player.email, exp: jwtExpiry }, process.env.JWT_SECRET);
...
app.post("points", authMiddleware, zValidator("json", pointInsertSchema), async (c) => {
    const body = await c.req.json();
...
app.get("high-scores", async (c) => {
    ...
    const profilePicUrlMap = await createProfilePicUrlMapFromHighScores(highScores);
    return c.json(highScores.map(({ players, points, level, outcomes }) => ({
...
app.put("player", authMiddleware, zValidator("json", playerUpdateSchema), async (c) => {
    const body = await c.req.json();
    await updatePlayer(c.get("playerId"), body);
    return c.json({ message: "Player updated successfully" });
});
app.put("game", authMiddleware, zValidator("json", gameUpdateSchema), async (c) => {
    const body = await c.req.json();
